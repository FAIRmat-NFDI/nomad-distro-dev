...
TODO: TOOLS = {k: v for k, v in config.north.tools.filtered_items()}
...
# TODO: put it into a fucntion and cache it. 
# TODO: Adding support to user specific tools 

TOOLS: list[NorthToolEntryPoint] = []
...
async def get_tools(
    ...
    # TODO: we should not have to loop over all plugins on every request. We can cache the north tools and only update the cache when the plugins are reloaded, which happens when the config file changes or when a new plugin is added.   
