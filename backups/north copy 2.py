...
##TODO: next release: about 2026-01-15
##TODO: finish 
...
def _get_status(tool: ToolModel, user: User, headers) -> ToolModel:
...
async def get_tools(user: UserDep, request: Request):
    ...
    return ToolsResponseModel(
        data=[
            _get_status(ToolModel(name=name, **tool.dict()), user, headers)
...
async def get_tool(
    ...
    logger.info(f"hub response:{response.json()}")
...
async def get_mounts(
    ...
    - uploads,
    - user specific "work" directorie(s),
